...
flowchart TB
%% ===== Styles (GitHub-safe) =====
classDef client fill:#111827,stroke:#38bdf8,color:#e2e8f0;
classDef edge fill:#0f172a,stroke:#38bdf8,color:#e2e8f0;
classDef k8s fill:#0b1320,stroke:#64748b,color:#e2e8f0;
classDef workload fill:#111827,stroke:#a78bfa,color:#e2e8f0;
classDef obs fill:#0b1320,stroke:#22c55e,color:#e2e8f0;
classDef autoscale fill:#111827,stroke:#f59e0b,color:#e2e8f0,stroke-dasharray: 6 4;

%% ===== Nodes =====
U["Client<br/>(curl / browser)"]:::client

ING["Ingress<br/>HTTP routing<br/>/api → app<br/>/health → app"]:::edge
SVC["Service<br/>LoadBalancer / ClusterIP<br/>single entrypoint"]:::edge

subgraph K8S["Kubernetes Cluster (Minikube)"]
direction TB

subgraph DEP["Deployment: sentiment-app<br/>replicas: 3 (base)"]
direction TB
P1["Pod #1<br/>Sentiment API<br/>:8080"]:::workload
P2["Pod #2<br/>Sentiment API<br/>:8080"]:::workload
P3["Pod #3<br/>Sentiment API<br/>:8080"]:::workload
end

HPA["HPA<br/>CPU-based autoscaling<br/>min:3  max:10"]:::autoscale

subgraph MON["Monitoring"]
direction TB
PR["Prometheus<br/>scrape + store metrics"]:::obs
GF["Grafana<br/>dashboards"]:::obs
SM["ServiceMonitor<br/>targets: service / pods"]:::obs
end
...
%% ===== Request flow =====
U -->|HTTP| ING
ING -->|/api/sentiment<br/>/health| SVC
SVC -->|load-balance| P1
SVC -->|load-balance| P2
SVC -->|load-balance| P3

%% ===== Probes (informational) =====
P1 -.->|"readiness/liveness<br/>probes"| SVC
P2 -.->|"readiness/liveness<br/>probes"| SVC
P3 -.->|"readiness/liveness<br/>probes"| SVC
